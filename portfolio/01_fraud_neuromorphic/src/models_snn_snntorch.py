...
class FraudSNNTorch(nn.Module):
    ...
    Architecture:
        ...
        - Hidden layers: Leaky neurons with learnable parameters
        - Output layer: 2 neurons (legitimate / fraudulent)
    
    Learning: Backpropagation Through Time (BPTT) with surrogate gradients
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        ...
        Returns:
            spk_out: Output spikes [time_steps, batch, output_size]
    ...
    def train_model(
        ...
        for epoch in epoch_pbar:
            ...
            for batch_idx, (data, targets) in enumerate(train_loader):
                ...
                # Forward pass
                spk_out, mem_out = self(data)
                
                # Loss calculation (spike count)
    ...
    def predict(
        ...
        Returns:
            ...
            spike_counts: Spike counts per output neuron
        """
        self.eval()
        x = x.to(device)
        ...
        with torch.no_grad():
            ...
            spike_counts = spk_out.sum(dim=0).squeeze()  # [output_size]
            
            # Predicted class
            predicted_class = spike_counts.argmax().item()
            
            # Confidence (softmax of spike counts)
            spike_probs = torch.softmax(spike_counts, dim=0)
            confidence = spike_probs[predicted_class].item()
        
        return predicted_class, confidence, spike_counts.cpu().numpy()
