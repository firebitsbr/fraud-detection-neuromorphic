#!/usr/bin/env python3
"""
Interactive Kaggle dataset setup with auto-detection.
    ...
    if file_type == 'zip':
        for zip_file in source_files:
            ...
            try:
                ...
                if delete == 's':
                    zip_file.unlink()
                    print_colored(f"🗑️  ZIP deletado: {zip_file.name}", GREEN)
        ...
        for csv_file in source_files:
            ...
            if target_file.exists():
                print_colored(f"   ⚠️  {csv_file.name} já existe", YELLOW)
...
def main():
    ...
    print_header("1️⃣  VERIFICANDO ARQUIVOS EXISTENTES")
    ...
    # Instructions
    print_header("2️⃣  INSTRUÇÕES PARA DOWNLOAD")
    ...
    # Auto-detect files
    print_header("3️⃣  DETECTANDO ARQUIVOS BAIXADOS")
    ...
    if total_found == 0:
        print_colored("\n⏱️  Timeout: Nenhum arquivo detectado automaticamente", YELLOW)
    ...
    # Display found files
    print_header("4️⃣  ARQUIVOS ENCONTRADOS")
    ...
    # Process files
    print_header("5️⃣  PROCESSANDO ARQUIVOS")
    ...
    # Final verification
    print_header("6️⃣  VERIFICAÇÃO FINAL")
    ...
    if missing:
        print_colored(f"\n⚠️  Ainda faltam: {len(missing)}", YELLOW)
        for filename, size in missing:
            print(f"   ✗ {filename}")
...
if __name__ == '__main__':
    try:
        ...
        print_colored("\n\n⚠️  Operação cancelada pelo usuário (Ctrl+C)", YELLOW)
        sys.exit(1)
    except Exception as e:
