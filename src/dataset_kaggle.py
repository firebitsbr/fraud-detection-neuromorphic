...
class FraudDatasetPreprocessor:
    ...
    def load_raw_data(self) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if cache_file.exists():
            print("⚡ Cache encontrado! Carregando dados pré-processados...")
        ...
        # Cache raw data for faster subsequent loads
...
def prepare_fraud_dataset(
    ...
    if num_workers is None:
        num_workers = min(8, mp.cpu_count())  # Cap at 8 to avoid overhead
    ...
    print(f"🔧 Workers: {num_workers} threads para DataLoader")
    ...
    with tqdm(total=5, desc="🔄 Pipeline de preparação", unit="etapa") as pbar:
        ...
        # Step 3: Preprocess
        pbar.set_description("⚙️ Preprocessando features")
        ...
        pbar.set_description("✂️ Dividindo train/val/test")
